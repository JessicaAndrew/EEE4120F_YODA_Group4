...
int main(void){
	...
	cl_mem imgMatrix_buffer, output_buffer, rowSize_buffer, colSize_buffer, countPixels_buffer, filterSize_buffer;
	...
	cl_kernel kernel = clCreateKernel(program, "medianFilter", &err); //select the kernel you are running
	...
	filterSize_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), &filterSize, &err);
	...
	clSetKernelArg(kernel, 5, sizeof(cl_mem), &filterSize_buffer);
	...
	clReleaseMemObject(filterSize_buffer);
	...
	printf ("Run Time of median filter (OpenCL): %0.8f sec \n", ((float) endProcess + startProcess)/CLOCKS_PER_SEC);
	printf ("Save to file Run Time: %0.8f sec \n", ((float) endPrint1 - startPrint1)/CLOCKS_PER_SEC);
	FILE *filePrintInfo = fopen("OutMedianOpenCLSpeed.txt","w");
